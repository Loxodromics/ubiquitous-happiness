...
#define PIXEL_PIN 0
#define MAX_ADC 1024
#define HOLD_TIMER 160
#define MAX_VALUE_SUM_COUNTER 20
#define RING_BUFFER_LENGTH 4
#define UPDATE_COUNT_DOWN 1200

// guessed values from office noice
#define LOWER_INTERVAL 5
#define UPPER_INTERVAL 150
...
void mapColor(int input, int brightness, u8 *red, u8 *green)
{
  ...
  {
    *green = brightness;
    *red = (input * brightness) / (NUM_PIXEL / 2);
  }
  else
  {
    *red = brightness;
    input -= NUM_PIXEL / 2;
    *green = brightness - ((input * brightness) / (NUM_PIXEL / 2));
  }
}
...
void setup() {
  ...
  updateCountDown = UPDATE_COUNT_DOWN;
}
...
void loop() {
  ...
  ringBuffer[ringBufferIndex] = (float) value;//lookup[lookupInAdjusted];

  float outValue = 0.0;
  for (int i = 0; i < RING_BUFFER_LENGTH; ++i) {
    outValue += ringBuffer[i] / RING_BUFFER_LENGTH;
  }


  int valuePixel = (outValue * NUM_PIXEL) / (MAX_ADC / 2);
  u8 red = 0;
  u8 green = 0;
  ...
  for (int eachPixel = 0; eachPixel < NUM_PIXEL; ++eachPixel)
  {
    if (eachPixel < valuePixel) {
      mapColor(eachPixel, 255, &red, &green);
    }
    else if (eachPixel < holdValue)
    {
      mapColor(eachPixel, (holdValue - valuePixel) + 1, &red, &green);
    }
    else {
      red = 0;
      green = 0;
