...
void loop() {
  ...
  int lookupInAdjusted = value - 25; // first x values seem to be garbage (always on)
  if (lookupInAdjusted < 0)
    lookupInAdjusted = 0;

  ringBuffer[ringBufferIndex] = (float) lookup[lookupInAdjusted];
